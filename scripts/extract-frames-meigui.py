#!/usr/bin/env python3
"""为玫瑰的故事全部 38 集抽取关键帧：每集 hero.jpg + 每场景 s{N}.jpg"""
import json, glob, os, subprocess, sys

CONTENT_DIR = '/Users/chenzhiheng/Projects/drama-analysis/content/meigui-de-gushi'
VIDEO_DIR = '/Users/chenzhiheng/Projects/drama-analysis/玫瑰的故事'
...
# 参数：可选指定集号（如 e03 或 03），默认全部
args = [a for a in sys.argv[1:]]
files = sorted(glob.glob(f'{CONTENT_DIR}/content-e*.json'))
if args:
    ep_arg = args[0].replace('e', '').zfill(2)
    files = [f for f in files if f.split('content-e')[1].split('.')[0] == ep_arg]
